   ...
   function getRssFeed(magazines) {
            ...
            async function getFeeds() {
                try{
                    ...
                    if (response.status === 200) {
                        ...
                        let isAccordionOpen = i === 0;
  
                        let accordion = renderAccordionSection(i, id, feed.title);
                        accordionSection.innerHTML += accordion;
                        
                        if(isAccordionOpen){
                          firstAccordianButton = document.querySelector(`#collaps-${id} .accordian-button`);
                        }
    ...
    if(firstAccordianButton){
      firstAccordianButton.addEventListener('click', () => {
          firstAccordianButton.click();
      });
      }
  }
    ...
    const renderAccordionSection = (index, id, title, isAccordionOpen) => {
      let divAccordion = `<div class="accordion-item">
                            <button class="accordion-button ${isAccordionOpen ? "" : "collapsed"}" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-${id}" aria-expanded="${isAccordionOpen}" aria-controls="collapse-${id}">
                              ${title}
                            </button>
                            <div id="collapse-${id}" class="accordion-collapse collapse ${index === 0 ? "show": ""}" aria-labelledby="heading-${id}" data-bs-parent="#accordionSection">
                              <div class="accordion-body">
                                <div id="indicator-${id}" class="carousel-container carousel slide" data-ride="carousel"> 
    ...
    const carouselItemTemplate = (el, j) => {
      let cardTemplate = `<div class="carousel-item ${j === 0 ? "active" : ""}">
                            <div class="card d-block">
                                <img class="vw-100 vh-50 img img-responsive" src=${el.enclosure.link} alt="${el.guid}">
                                <div class="card-body">
